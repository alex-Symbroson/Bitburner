...
async function daemon(ns)
{
    ...
    const maxRam = 20;
    let multi = 4; // assumes you need up to 8gb for your hack and distro script. you may be able to lower this accordingly.
    ...
    while (multi < maxRam)
    {
        ...
        if (p.money < cost) await ns.asleep(3000);
        else if (queue.length >= sd.srvLimit)
        {
